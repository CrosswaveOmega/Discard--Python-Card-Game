...
async def make_tiebreaker_double(ctx, choices, message=None, timeout_enable=False, delete_after=False, remove_after=False, clear_after=False, ignore_message=False, ignore_reaction=False): #Add card.
    '''
    ...
    async def getTwoReactions():
        ...
        while running:
            ...
            if middleB == False:
                tasklist2.append(rb)
            done, pending = await asyncio.wait(tasklist2, return_when=asyncio.FIRST_COMPLETED) #there's probably a better way to do this.
